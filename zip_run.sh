...
#VALGRIND="valgrind --tool=memcheck --leak-check=yes --track-origins=yes -s"
...
hw_dfl_hw_ifl()
{
	...
	hw_blk_inflate /tmp/ori.gz origin gzip -b 8192
	md5sum -c ori.md5
	echo "Pass RANDOM data for hw compress and hw decompress in BLOCK"

	${RM} -f /tmp/ori.gz
	hw_strm_deflate origin /tmp/ori.gz gzip -b 8192
	hw_strm_inflate /tmp/ori.gz origin gzip -b 8192
	md5sum -c ori.md5
	echo "Pass RANDOM data for hw compress and hw decompress in STREAM"
